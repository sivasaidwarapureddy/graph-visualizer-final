...
const convertToFlow = (graph) => {
  ...
  graph.edges.forEach((e) => {
    ...
    if (!graph.directed) adj[e.destination].push(e.source);
  });
  ...
  const levelMap = {};
  for (let i = 0; i < n; i++) {
    if (!levelMap[level[i]]) levelMap[level[i]] = [];
...
const animate = async (steps) => {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    // 🔵 BFS → USE QUEUE FROM BACKEND
    if (mode === "BFS") {
      setStructure(steps[i].queue || []);
    }

    // 🟣 DFS → USE STACK FROM BACKEND
    else {
    ...
    setNodes((nds) =>
      nds.map((n) => {
        ...
        return {
          ...
          style: {
            ...n.style,
            background: isVisited ? "#f97316" : "#1e293b",
            border: isCurrent
              ? "3px solid #fb923c"
              : "2px solid #334155",
            boxShadow: isCurrent
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-900 text-white overflow-hidden">
      ...
      <div className="flex gap-4 justify-center mb-4">
        ...
        <button onClick={handleBFS} disabled={!graph || isRunning} className="btn">
          Run BFS
        </button>

        <button onClick={handleDFS} disabled={!graph || isRunning} className="btn">
          Run DFS
        </button>
      </div>
  ...
  <div className="w-[140px] flex flex-col items-center">
    ...
    <div
      className={`border-2 border-green-400 w-full flex ${
        mode === "BFS" ? "flex-col" : "flex-col-reverse"
      } items-center bg-slate-800 rounded-lg max-h-[70vh] overflow-y-auto`}
    >

      {/* FRONT */}
      {mode === "BFS" && (
        <div className="text-xs text-green-400 py-1">FRONT</div>
      )}
      ...
      {/* BACK */}
      {mode === "BFS" && (
        <div className="text-xs text-green-400 py-1">BACK</div>
      )}
